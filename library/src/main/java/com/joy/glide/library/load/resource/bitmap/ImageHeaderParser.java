...
import static com.joy.glide.library.load.resource.bitmap.ImageHeaderParser.ImageType.GIF;
import static com.joy.glide.library.load.resource.bitmap.ImageHeaderParser.ImageType.JPEG;
import static com.joy.glide.library.load.resource.bitmap.ImageHeaderParser.ImageType.PNG;
import static com.joy.glide.library.load.resource.bitmap.ImageHeaderParser.ImageType.PNG_A;
import static com.joy.glide.library.load.resource.bitmap.ImageHeaderParser.ImageType.UNKNOWN;
...
public class ImageHeaderParser {
		...
		while (true) {
			...
			if (segmentType != EXIF_SEGMENT_TYPE) {
				...
				if (skipped != segmentLength) {
					if (Log.isLoggable(TAG, Log.DEBUG)) {
						Log.d(TAG, "Unable to skip enough data" + ", type: " + segmentType + ", wanted to skip: " + segmentLength + ", but actually " +
								"skipped: " + skipped);
					}
					return null;
				}
				...
				if (read != segmentLength) {
					if (Log.isLoggable(TAG, Log.DEBUG)) {
						Log.d(TAG, "Unable to read segment data" + ", type: " + segmentType + ", length: " + segmentLength + ", actually read: " +
								read);
					}
					return null;
				} else {
	...
	private static boolean handles(int imageMagicNumber) {
		return (imageMagicNumber & EXIF_MAGIC_NUMBER) == EXIF_MAGIC_NUMBER || imageMagicNumber == MOTOROLA_TIFF_MAGIC_NUMBER || imageMagicNumber ==
