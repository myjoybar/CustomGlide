...
/**
 * A general purpose size limited cache that evicts items using an LRU algorithm. By default
 * every item is assumed to
...
public class LruCache<T, Y> {
    ...
    /**
     * Sets a size multiplier that will be applied to the size provided in the constructor to set
     * the new size of the
     * cache. If the new size is less than the current size, entries will be evicted until the
     * current size is less
    ...
    /**
     * Returns the size of a given item, defaulting to one. The units must match those used in
     * the size passed in to the
     ...
     * the cache and
     * instead {@link #onItemEvicted(Object, Object)} will be called synchronously with the given
     * key and item.
     * </p>
     *
    ...
    /**
     * Removes the item at the given key and returns the removed item if present, and null
     * otherwise.
     *
    ...
    /**
     * Removes the least recently used items from the cache until the current size is less than
     * the given size.
     *
